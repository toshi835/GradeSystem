...
def test(args):
    ...
    x, y = data_loader(
        mode="test", FEA_PATH=FEA_PATH, EMBED_PATH=EMBED_PATH, wo_ngram=args.wo_ngram
    )
...
def test_bert(args):
    ...
    dataset_test = CreateDataset(
        PATH + "test.json",
        feature_file_path=FEA_PATH + "train/test.csv" if args.feature else "",
        gec_file_path=GEC_PATH + "test.csv" if GEC_PATH else "",
    )  # "train_wi.json")
    test_loader = DataLoader(dataset=dataset_test, batch_size=32)

    split_num = dataset_test.split_num
    if args.feature:
        model = DebertaClass(split_num, feature_length=len(dataset_test[0]["feature"]))
    else:
...
def test_raw(args):  # lr w/o GECのみ可能
    ...
    if args.clf == "lr":
        ...
        print("preprocessed time: {:.2f}s".format(preprocessed_time - start_time))
        ...
        model.eval()
        pre2_time = time.time()
        print("loading time: {:.2f}s".format(pre2_time - preprocessed_time))

        src = tokenizer.encode_plus(src_str, add_special_tokens=True)
        with torch.no_grad():
